...
@section('title', 'Acadeimc Sections')

@section('page-content')
    <main class="flex-grow flex flex-col min-w-0  overflow-y-auto">
        <x-loader-component />

        <div data-live-search data-search-url="{{ route('sections.index') }}" data-search-delay="300">
            <x-dashboard-header>
                <div class="flex items-center gap-4 flex-grow max-w-xl">
                    <div class="relative w-full">
                        <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
                        <input data-search-input type="text" placeholder="Search sections..."
            ...
            <div class="flex-1 overflow-y-auto">
                <div class="p-4 md:p-8">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <h1 class="text-xl font-extrabold text-primary">Academic Sections</h1>
                            <p class="text-slate-500 text-xs">Manage sections and thier levels.</p>
                        </div>
                    ...
                    <div class="space-y-4" data-search-results>
                        @include('academic.sections.partials.rows', ['sections' => $sections])
                    </div>
                    <div class="flex justify-center mt-4" data-search-pagination>
                        @include('academic.sections.partials.pagination', ['sections' => $sections])
                    </div>
                </div>
            </div>
        </div>
