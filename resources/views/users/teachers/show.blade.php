    ...
    <main class="flex-1 overflow-y-auto p-4 md:p-6">
            <div class="max-w-4xl mx-auto">
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <a href="{{ route('teachers.index') }}" class="text-accent text-xs mb-2 flex items-center gap-1 hover:underline">
                            <i class="fas fa-arrow-left text-xs"></i> Back to Teachers
                        </a>
                        <h1 class="text-xl font-bold text-slate-800">{{ $teacher->user->name() }}</h1>
                    ...
                    <div class="bg-white rounded-lg border border-slate-200 p-4">
                       ...
                       <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-800 mt-1">{{ str_replace('_', ' ', strtoupper($teacher->employment_type)) }}</p>
                            </div>
                ...
                <div class="flex gap-3 flex-wrap">
                    <a href="{{ route('user.edit-password', $teacher->user_id) }}" class="px-4 py-2 bg-slate-600 text-white text-xs rounded font-medium hover:bg-slate-700 transition flex items-center gap-2">
                        <i class="fas fa-key text-xs"></i> Change Password
                    </a>
                    <a href="{{ route('teachers.index') }}" class="px-4 py-2 bg-slate-300 text-slate-800 text-xs rounded font-medium hover:bg-slate-400 transition flex items-center gap-2">
